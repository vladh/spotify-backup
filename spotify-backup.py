...
def main():
    ...
    parser.add_argument(
        '--token', metavar='OAUTH_TOKEN', help='use a Spotify OAuth token (requires the ' +
        '`playlist-read-private` permission)'
    )
    parser.add_argument('--format', default='tsv', choices=['json', 'tsv'], help='output format (default: tsv)')
    parser.add_argument('file', help='output filename', nargs='?')
    args = parser.parse_args()

    # If they didn't give a filename, then just prompt them. (They probably just double-clicked.)
    while not args.file:
        args.file = input('Enter a file name (e.g. playlists.tsv): ')
            ...
            for playlist in playlists:
                ...
                for track in playlist['tracks']:
                    if track['track']:
                        ...
                        log(f'Could not load track {str(track)}.')
                f.write('\r\n')
